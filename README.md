...
- **`Alt+1~9`**：始终按侧边栏显示顺序切换到第 N 个会话（与固定无关，所见即所得——分组折叠、活动提升后的顺序都自动跟随）。
- **`Alt+Shift+1~9`**：独立的固定槽位三态键——空槽位固定当前会话；固定着别的会话时跳转过去；固定着当前会话时取消固定。适合"很多频繁交互的会话，一键回到之前的对话"。
- **`Ctrl+Alt+1~9`**：纯跳转到固定槽位 N，不改变任何固定（带 AltGr 防护）。
- **`Alt+N`**：新建会话（当前工作区，否则最近工作区）。
- **`Alt+\``**（数字 1 左边的键）：会话导航模式——在**真实侧边栏会话行**上移动高亮环，`↑↓` 选择、`Enter` 进入（等同点击该行）、`Esc` 取消，未按 Enter 不切换会话。
- **`Alt+P`**：打开/关闭快捷键面板。
- **`Alt+Shift+F`**：聚焦会话搜索框并清空当前内容（侧边栏收起时自动展开）。
- **所有键位可自定义**：面板「按键」页录制式改键，冲突检测、一键恢复默认；键位与固定关系都保存在本浏览器（localStorage），刷新/重启 DSH 后依然有效。
- **面板三个 Tab**：「Alt+1-9」顺序列表（可给任意会话选择固定到具体槽位）/「Alt+Shift」固定槽位管理 /「按键」改键 + 速查说明（每行一句话说明，悬停看完整详情）。
