...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const KEY_SECTIONS = [
      ["secCommon", ["navMode", "switchDigit", "prevSession", "nextSession", "newSession", "panel", "focusModel", "inputFocus", "sendAlternate"]],
    ...
    function apply(ctx) {
        ...
        try {
          ...
          for (const id of Object.keys(out)) {
            ...
            if (out[id].digit === true) {
              ...
              const ok = /^Key[A-Z]$/.test(spec.code) || spec.code === "Backquote" || spec.code === "Enter" || /^Arrow(Up|Down|Left|Right)$/.test(spec.code) || /^F([1-9]|1[0-2])$/.test(spec.code);
              if (!ok) continue;
              out[id] = {
                ctrl: spec.ctrl === true, alt: spec.alt === true, shift: spec.shift === true, meta: spec.meta === true,
      ...
      const handleModelMenuKey = (event) => {
        ...
        if (active instanceof Node && menu.contains(active)) return false; // inside the menu: DSH handles it
        if (event.key === "Escape") {
          event.preventDefault();
          event.stopPropagation();
          try { trigger.click(); } catch {}
          window.setTimeout(() => ringFocus(trigger), 0);
          return true;
        }
