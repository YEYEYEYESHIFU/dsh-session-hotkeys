  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      const startNav = () => {
        ...
        const tryStart = (left) => {
          ...
          if (fresh !== null && fresh.items.length > 0) {
            ...
            scrollToNavRow();
            showToast("导航模式 · " + fresh.items.length + " 个目标 · ↑↓ 移动，Enter 进入");
            return;
          }
      ...
      const expandOverflow = () => {
        ...
        const retry = (left) => {
          ...
          if (fresh !== null && fresh.items.length > 0) {
            ...
            scrollToNavRow();
            showToast(expanding ? "已展开 · 高亮已移到首个新会话，按 Enter 进入" : "已收起该组");
            return;
          }
          if (left > 0) window.setTimeout(() => retry(left - 1), 90);
          else { exitNav(); showToast("会话列表更新失败", "warn"); }
        };
      ...
      const confirmNav = () => {
        ...
        if (!nav.active) return;
        const item = nav.items[nav.index];
        if (item === undefined) { showToast("目标行已不存在", "warn"); return; }
