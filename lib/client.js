...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const ACTION_LABELS = {
      switchDigit: "Alt+1-9 顺序切换",
      pinToggle: "Alt+Shift 固定槽位",
      jumpPin: "Ctrl+Alt 跳转固定",
      newSession: "新建会话",
      navMode: "导航模式",
      panel: "打开面板",
      searchFocus: "聚焦搜索"
    ...
    const CSS = [
      ...
      ".shk-helprow{display:flex;align-items:center;gap:8px;padding:3px 6px;border-radius:8px;min-height:30px}",
      ".shk-helprow:hover{background:var(--dsw-alias-interactive-bg-hover)}",
      ".shk-helpkey{flex:none;width:96px;box-sizing:border-box;font-family:ui-monospace,Consolas,monospace;font-size:10px;font-weight:700;color:var(--dsw-alias-brand-primary);padding:2px 0;border:1px solid var(--dsw-alias-border-l1);border-radius:6px;background:var(--dsw-alias-bg-layer-2);text-align:center;white-space:nowrap}",
      ".shk-helpdesc{flex:1;min-width:0;font-size:10px;color:var(--dsw-alias-label-primary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
      ...
      ".shk-keyrow{display:flex;align-items:center;gap:8px;padding:3px 6px;border-radius:8px;min-height:30px}",
    ...
    function apply(ctx) {
      ...
      const readBindings = () => {
        const out = {};
        for (const id of Object.keys(DEFAULT_BINDINGS)) out[id] = { ...DEFAULT_BINDINGS[id] };
        try {
      ...
      const pinSession = (sessionId, slot) => {
        ...
        writePins();
        pinsBus.set({ ...pins });
        showToast("已固定 Alt+Shift+" + slot + " ← " + titleOf(sessionId));
      };
      const pinToggle = (n) => {
        const s = snap();
        const key = String(n);
        ...
        if (pinnedId === undefined) {
          ...
          writePins();
          pinsBus.set({ ...pins });
          showToast("已固定 Alt+Shift+" + n + " ← " + titleOf(current));
          return;
        }
      ...
      const resetBindings = () => {
        bindings = {};
        for (const id of Object.keys(DEFAULT_BINDINGS)) bindings[id] = { ...DEFAULT_BINDINGS[id] };
        writeBindings();
      ...
      function OverlayView(props) {
        ...
        if (ui.open === true && ui.rect !== null) {
          ...
          for (let n = 1; n <= SLOT_COUNT; n++) {
            ...
            posRows.push(React.createElement("div", { key: "p" + n, className: "shk-row" },
              ...
              React.createElement("button", {
                ...
                title: pinnedSlot !== 0 ? "已固定在 Alt+Shift+" + pinnedSlot + " · 点击改槽位" : "固定到 Alt+Shift+1-9 的某个槽位",
                onClick: () => {
            ...
            if (pickerFor === n && pid !== undefined) {
              ...
              for (let k = 1; k <= SLOT_COUNT; k++) {
                ...
                picks.push(React.createElement("button", {
                  ...
                  title: occupied ? "槽位 " + k + " 已占用（点击替换）" : "固定到 Alt+Shift+" + k,
          ...
          for (let n = 1; n <= SLOT_COUNT; n++) {
            ...
            pinRows.push(React.createElement("div", { key: "s" + n, className: "shk-row" },
              ...
              React.createElement("button", {
                ...
                title: pinnedId === undefined ? "空槽位（Alt+Shift+" + n + " 固定当前会话）" : "Alt+Shift+" + n + " 跳转 · 点击切换",
                onClick: () => {
          ...
          for (const id of Object.keys(DEFAULT_BINDINGS)) {
            ...
            keyRows.push(React.createElement("div", { key: "k-" + id, className: "shk-keyrow" },
              React.createElement("span", { className: "shk-keyname" }, isRec ? "正在录制：请按新组合键（Esc 取消）…" : ACTION_LABELS[id]),
          ...
          keyRows.push(React.createElement("div", { key: "k-reset", className: "shk-tipbox" },
            React.createElement("span", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" } },
              "键位保存在本浏览器，重启 DSH 后依然有效。",
              React.createElement("button", { className: "shk-mini", title: "恢复默认键位", onClick: resetBindings }, "恢复默认")
            )
          ));
          const helpSections = [
            ["切换与固定", [
              ["Alt+1–9", "按显示顺序切换第 N 个会话", "始终按侧边栏显示顺序切换第 N 个会话（与固定无关）"],
              ["Alt+Shift+1–9", "槽位三态：固定 / 跳转 / 取消", "固定槽位三态键：空槽位→固定当前会话；固定着别的会话→跳转过去；固定着当前会话→取消固定"],
              ["Ctrl+Alt+1–9", "纯跳转到固定槽位", "直接跳转到固定槽位 N，不改变任何固定（AltGr 除外）"]
            ]],
            ["新建与面板", [
              ["Alt+N", "新建会话", "在当前工作区（否则最近工作区）新建会话"],
              ["Alt+P", "打开 / 关闭本面板", "打开或关闭本快捷键面板"]
            ]],
            ["导航与搜索", [
              ["Alt+`", "导航模式：↑↓ 选择 · Enter 进入", "进入会话导航模式：↑↓ 在会话行上移动高亮，Enter 进入（等同点击该行），Esc 或其它按键退出"],
              ["Alt+Shift+F", "聚焦并清空搜索框", "聚焦会话搜索框并清空当前搜索内容"]
            ]]
          ];
          const helpRows = [];
          for (const [secTitle, items] of helpSections) {
            helpRows.push(React.createElement("div", { key: "sec-" + secTitle, className: "shk-helpsec" }, secTitle));
            for (const [keyText, desc, full] of items) {
              helpRows.push(React.createElement("div", { key: "h-" + keyText, className: "shk-helprow", title: full },
                React.createElement("span", { className: "shk-helpkey" }, keyText),
                React.createElement("span", { className: "shk-helpdesc" }, desc)
              ));
            }
          }
          helpRows.push(React.createElement("div", { key: "h-tip", className: "shk-tipbox" },
            "所有键位都可以在「按键」页自定义。固定槽位与顺序列表完全独立，固定关系保存在本浏览器（刷新不丢）。"));
          const width = 312;
          const height = 500;
          ...
          children.push(React.createElement("div", {
            ...
            React.createElement("div", { className: "shk-tabs" },
              ...
              React.createElement("button", { className: "shk-tab" + (tab === "help" ? " shk-tab-on" : ""), onClick: () => { setTab("help"); setPickerFor(0); } }, "帮助")
            ),
            React.createElement("div", { className: "shk-rows" }, tab === "pos" ? posRows : tab === "pin" ? pinRows : tab === "keys" ? keyRows : helpRows),
            tab === "pos" || tab === "pin" ? React.createElement("div", { className: "shk-hint" }, tab === "pos"
              ? "Alt+1-9 始终按显示顺序切换 ·「固定」把该会话放入独立的 Alt+Shift 槽位"
              : "Alt+Shift+N：空→固定当前 / 指着别的→跳转 / 指着当前→取消 · Ctrl+Alt+N 纯跳转") : null,
