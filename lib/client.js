  ...
  factory: (require) => {
    ...
    const I18N = {
      zh: {
        ...
        "desc.closeSplitView": "关闭（隐藏）或恢复 dsh-split-view 分屏工作区",
        ...
        "full.closeSplitView": "关闭（隐藏）dsh-split-view 分屏工作区，回到原生单会话视图；再按一次恢复。分屏只是被隐藏——面板、各窗格与其会话都保持挂载，不会被关闭或归档；选择会记住，刷新页面后仍保持关闭状态",
      ...
      en: {
        ...
        "label.closeSplitView": "Close / restore split view",
        ...
        "desc.closeSplitView": "Close (hide) or restore the split view workspace",
        ...
        "full.closeSplitView": "Closes (hides) the dsh-split-view workspace and returns to the native single-conversation view; press again to restore. The workspace is only hidden — the panes and their sessions stay mounted, nothing is closed or archived. The choice is remembered and survives a page refresh",
        ...
        "dsvClosed": "Split view closed · press again to restore",
    ...
    function apply(ctx) {
      ...
      // ---------- split view close / restore ----------
      // Alt+W / ⌃⌥W: hide or restore the dsh-split-view workspace. The plugin
      // is always-on and ships no exit of its own, so this toggles a root
      // class that display-hides its .dsv-overlay — the native single-chat
      // view underneath becomes fully interactive again while the workspace
      // (and every pane's session) stays mounted, merely hidden. The choice
      // persists in localStorage and is re-applied at startup, so a refresh
      // never resurrects a view the user closed.
      ...
      const closeSplitView = () => {
        ...
        showToast(t(hidden ? "dsvClosed" : "dsvRestored"));
      };
