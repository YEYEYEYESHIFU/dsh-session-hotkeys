...
//   Esc                      return focus to the chat transcript from anywhere — closes the panel,
//                              exits nav, closes the model menu; in the input, popups and IME keep their Esc
  ...
  factory: (require) => {
    ...
    const I18N = {
      zh: {
        ...
        "desc.inputBlur": "按 Esc，无论焦点在哪都交还给聊天内容区",
        ...
        "full.inputBlur": "按 Esc（默认）总是把键盘焦点交还给聊天内容滚动区（首次自动加 tabindex=-1 使其可聚焦，并抑制其焦点外框），↑↓ / PgUp / PgDn / Home / End 随即原生滚动会话内容；途中顺带取消当前临时状态：快捷键面板关闭、导航模式退出、模型菜单关闭（焦点不再弹回触发按钮）、改键录制照旧取消。聊天输入框聚焦时，斜杠命令弹层与 IME 组合仍拥有 Esc；草稿与光标永远不动。可换绑（按 Esc 本身会取消录制——用「恢复默认」找回）",
      ...
      en: {
        ...
        "desc.inputBlur": "Esc anywhere hands focus back to the chat",
    ...
    const CSS = [
      ...
      "[data-conversation-scroll][tabindex='-1']:focus,[data-conversation-scroll][tabindex='-1']:focus-visible{outline:none!important}"
    ].join("\n");
    ...
    function apply(ctx) {
      ...
      const focusChatTranscript = () => {
        ...
        for (const selector of CHAT_SCROLLER_SELECTORS) {
          try { scroller = document.querySelector(selector); } catch { continue; }
          if (scroller !== null) break;
        }
        if (scroller === null) { showToast(t("chatFocusNone"), "warn"); return false; }
        if (document.activeElement === scroller) return true; // already there: stay silent
        try {
        ...
        try {
          ...
          if (blurMatches(event) && !composerPopupOpen()) {
            ...
            if (input === null || document.activeElement !== input) {
              let scroller = null;
              try { scroller = document.querySelector(CHAT_SCROLLER_SELECTORS[0]); } catch {}
              if (scroller !== null) focusChatTranscript();
            }
          }
