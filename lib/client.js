...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const KEY_SECTIONS = [
      ["切换与固定", [
        ["switchDigit", "Alt+1-9 顺序切换", "按显示顺序切换第 N 个会话", "始终按侧边栏显示顺序切换第 N 个会话（与固定无关）"],
        ["pinToggle", "Alt+Shift 固定槽位", "槽位三态：固定 / 跳转 / 取消", "固定槽位三态键：空槽位→固定当前会话；固定着别的会话→跳转过去；固定着当前会话→取消固定"],
        ["jumpPin", "Ctrl+Alt 跳转固定", "纯跳转到固定槽位", "直接跳转到固定槽位 N，不改变任何固定（AltGr 除外）"]
      ]],
      ["新建与面板", [
        ...
        if (ui.open === true && ui.rect !== null) {
          ...
          keyRows.push(React.createElement("div", { key: "k-reset", className: "shk-tipbox" },
            React.createElement("span", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" } },
              "键位与固定关系都保存在本浏览器（刷新/重启后依然有效）。悬停每行可看完整说明。",
