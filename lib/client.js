...
//   Alt+W / ⌃⌥W              close (hide) the split view workspace; press again to restore
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const DSV_HIDDEN_KEY = "dsh.session-hotkeys.dsvHidden";
    ...
    // The composer's chat input. Up to DSH 0.1.1 it is a textarea carrying
    // data-phase, always inside the data-composer-card wrapper (the feedback /
    // question modals' textareas carry neither). DSH 0.1.5 replaced it with a
    // Lexical contenteditable div: class-hashed, but stably marked with
    // data-composer-input (+ data-phase, role=textbox). While empty its
    // innerText/textContent are "" — the placeholder is only a
    // data-placeholder attribute rendered via CSS — so emptiness checks stay
    // reliable (see draftTextOf).
    const COMPOSER_INPUT_SELECTORS = [
      ...
      'textarea[data-dsv-tui-input]',
      '.dsv-tui-input'
    ];
    // The sidebar's collapsed rail. DSH <= 0.1.1 exposed data-sidebar-collapsed;
    // 0.1.5 dropped the attribute and marks the collapsed sidebar column with
    // the hash-tolerant _root + _collapsed class pair (same convention the rail
    // CSS below uses). Check both so the plugin works on either core.
    const SIDEBAR_COLLAPSED_SELECTOR = "[data-sidebar-collapsed], [class*='_root'][class*='_collapsed']";
    ...
    const WIN_DEFAULTS = {
      ...
      closeSplitView: { ctrl: false, alt: true, shift: false, meta: false, digit: false, code: "KeyW" },
    ...
    const MAC_DEFAULTS = {
      ...
      closeSplitView: { ctrl: true, alt: true, shift: false, meta: false, digit: false, code: "KeyW" },
    ...
    const I18N = {
      zh: {
        ...
        "label.closeSplitView": "关闭聚焦面板 / 退出分屏",
        ...
        "desc.closeSplitView": "关闭聚焦的分屏面板；只剩一个时收起整个分屏",
        ...
        "full.closeSplitView": "关闭当前聚焦的窗格：三个面板→两个、两个→一个（被关面板的会话不受影响，仍留在侧边栏）。当只剩最后一个面板时，收起整个分屏工作区、回到原生单会话视图——面板树被保留，再按一次 Alt+W 或点右下角的「分屏」角标即可原样恢复；收起状态会记住，刷新页面后同样保持",
        ...
        "dsvPaneClosed": "已关闭面板 · 剩 {n} 个",
        "dsvExited": "已退出分屏 · 再按一次恢复",
        "dsvRestored": "已恢复分屏",
        "dsvMissing": "分屏未开启",
        "dsvRestoreBtn": "分屏",
        "dsvRestoreTitle": "显示分屏工作区（Alt+W）",
        ...
        "copyFailed": "复制失败",
        "archivedNoRestore": "⚠️ DSH 0.1.x 没有公开的取消归档接口：浏览器端无法恢复会话。复制会话 ID 可配合宿主端插件或手动恢复；归档不会删除会话日志。",
      ...
      en: {
        ...
        "label.closeSplitView": "Close focused pane / exit split view",
        ...
        "desc.closeSplitView": "Close the focused pane; the last one tucks the workspace away",
        ...
        "full.closeSplitView": "Closes the focused pane: three panes → two, two → one (the closed pane's session is untouched and stays in the sidebar). When only one pane remains, the whole workspace tucks away and the native single-conversation view returns — the pane tree is preserved, so pressing Alt+W again or clicking the corner \u201cSplit view\u201d pill restores it exactly as it was. The tucked-away state is remembered and survives a page refresh",
        ...
        "dsvPaneClosed": "Pane closed · {n} left",
        "dsvExited": "Split view closed · Alt+W restores",
        "dsvRestored": "Split view restored",
        "dsvMissing": "Split view is not open",
        "dsvRestoreBtn": "Split view",
        "dsvRestoreTitle": "Show the split view workspace (Alt+W)",
        ...
        "archivedNoRestore": "⚠️ DSH 0.1.x has no public unarchive API: sessions cannot be restored from the browser. Copy the session id for a host-side plugin or manual restore; archiving never deletes session logs.",
    ...
    const KEY_SECTIONS = [
      ["secCommon", ["navMode", "switchDigit", "prevSession", "nextSession", "newSession", "panel", "toggleSidebar", "closeSplitView", "focusModel", "inputFocus", "inputBlur", "sendAlternate"]],
    ...
    const CSS = [
      ...
      ".shk-silent:focus,.shk-silent:focus-visible{outline:none!important;outline-offset:0!important;box-shadow:none!important}",
      ".dsv-pane:has(.shk-silent:focus),.dsv-pane:has(.shk-silent:focus-visible){outline:none!important;box-shadow:none!important}",
      // Rail footer fix: the official sidebar shell lays sidebar.footer.action
      // entries out as ONE horizontal row even in the collapsed 56px rail, so
      // every plugin icon piles up sideways and clips. Stack them vertically in
      // the rail instead. Hash-tolerant [class*=_attr] selectors (same
      // convention as above) survive CSS-module hash churn across upgrades;
      // (0,2,1) specificity beats the shell\'s (0,2,0) collapsed rules.
      "div[class*='_root'][class*='_collapsed'] div[class*='_footerActions']{flex-direction:column;align-items:center;gap:2px;width:auto}",
      "div[class*='_root'][class*='_collapsed'] div[class*='_footerActions']>*{flex:none;max-width:100%}",
      // closeSplitView (Alt+W): hide the dsh-split-view workspace. The class
      // sits on <html> so it applies regardless of mount order, and survives
      // the overlay being re-rendered (the rule keeps matching).
      "html.shk-dsv-hidden .dsv-overlay{display:none!important}",
      // corner pill shown while the split view workspace is tucked away
      ".shk-dsv-restore{position:fixed;right:14px;bottom:14px;z-index:2050;display:inline-flex;align-items:center;gap:6px;padding:4px 12px;border:1px solid var(--dsw-alias-border-l1);border-radius:999px;background:var(--dsw-alias-bg-overlay);color:var(--dsw-alias-label-secondary);font-size:11px;font-weight:600;font-family:inherit;cursor:pointer;box-shadow:0 2px 10px rgba(0,0,0,.18);transition:color .12s ease,border-color .12s ease}",
      ".shk-dsv-restore:hover{color:var(--dsw-alias-brand-primary);border-color:var(--dsw-alias-brand-primary)}"
    ].join("\n");

    function apply(ctx) {
      // All four are declared in `exports.inject` (see the bottom of the file),
      // so cordis has them live before apply runs — on both 0.1.1 and 0.1.5.
      // The guards below stay as a safety net for exotic hosts.
      ...
      // Start-session lives in different places across cores: DSH <= 0.1.1 had
      // it on the `workspaces` service, 0.1.5 moved it to `uiWorkspace` (the
      // `workspaces` service keeps archiveSession/list). Resolve lazily at
      // keypress time so either core works regardless of service arrival order.
      const startSessionFn = () => {
        ...
        if (uiWorkspace !== undefined && uiWorkspace !== null && typeof uiWorkspace.startSession === "function") {
          return () => { uiWorkspace.startSession(); };
        }
        if (workspaces !== undefined && typeof workspaces.startSession === "function") {
          return () => { workspaces.startSession(); };
        }
        return null;
      };
      const newSession = () => {
        const start = startSessionFn();
        if (start === null) { showToast(t("svcWorkspacesUnavailable"), "warn"); return; }
        try {
          start();
      ...
      const focusSearch = () => {
        const collapsed = document.querySelector(SIDEBAR_COLLAPSED_SELECTOR) !== null;
        if (collapsed) {
      ...
      // ---------- split view pane close / workspace restore ----------
      // Alt+W / ⌃⌥W: close the FOCUSED pane through the split view's bridge —
      // three panes → two, two → one. When a single pane remains it is kept
      // and { exit: true } comes back instead: the whole workspace shell is
      // then display-hidden (root class, persisted in localStorage), the
      // native single-chat view becomes fully interactive, and a corner pill
      // (plus Alt+W itself) restores the workspace with its pane tree intact.
      // The pill lives on document.body so the SPA can never drop it, and
      // applyDsvHidden keeps it in sync with the hidden state.
      const dsvHiddenPreferred = () => {
        try { return window.localStorage.getItem(DSV_HIDDEN_KEY) === "1"; } catch { return false; }
      };
      const applyDsvHidden = (hidden) => {
        try { window.localStorage.setItem(DSV_HIDDEN_KEY, hidden ? "1" : "0"); } catch {}
        try { document.documentElement.classList.toggle("shk-dsv-hidden", hidden); } catch {}
        try {
          let pill = document.getElementById("shk-dsv-restore");
          if (hidden && pill === null) {
            pill = document.createElement("button");
            pill.id = "shk-dsv-restore";
            pill.className = "shk-dsv-restore";
            pill.type = "button";
            pill.textContent = "\u29c9 " + t("dsvRestoreBtn");
            pill.title = t("dsvRestoreTitle");
            pill.addEventListener("click", () => restoreSplitView());
            document.body.appendChild(pill);
          } else if (!hidden && pill !== null) {
            pill.remove();
          }
        } catch {}
      };
      applyDsvHidden(dsvHiddenPreferred());
      const restoreSplitView = () => {
        applyDsvHidden(false);
        showToast(t("dsvRestored"));
      };
      const closeSplitView = () => {
        // Tucked away: this key brings the workspace back (its pane tree was
        // preserved on exit, so the panes return exactly as they were).
        if (dsvHiddenPreferred() === true) {
          if (document.querySelector(".dsv-overlay") !== null) { restoreSplitView(); return; }
          showToast(t("dsvMissing"), "warn");
          return;
        }
        // Preferred path: the split view's own bridge closes the focused pane
        // and reports how many panes remain (0 = exit → tuck the shell away).
        try {
          const bridge = window.__dshSplitView;
          if (bridge !== undefined && bridge !== null && typeof bridge.closeFocused === "function") {
            const result = bridge.closeFocused();
            if (result !== null && result !== undefined) {
              if (result.exit === true) applyDsvHidden(true);
              showToast(t(result.exit === true ? "dsvExited" : "dsvPaneClosed", { n: result.remaining }));
              return;
            }
          }
        } catch (err) { console.error("dsh-session-hotkeys closeSplitView:", err); }
        // Legacy fallback: an older dsh-split-view without the bridge — fall
        // back to the plain workspace hide/restore toggle.
        const overlay = document.querySelector(".dsv-overlay");
        if (overlay === null && dsvHiddenPreferred() === false) { showToast(t("dsvMissing"), "warn"); return; }
        const hidden = !dsvHiddenPreferred();
        applyDsvHidden(hidden);
        showToast(t(hidden ? "dsvExited" : "dsvRestored"));
      };

      // True when the element is actually rendered (not inside a display:none
      // subtree). The split-view workspace keeps its panes MOUNTED while the
      // whole workspace is tucked away (Alt+W hides .dsv-overlay), so presence
      // in the DOM is not enough — focusing a hidden pane's composer/transcript
      // silently fails (focus falls to <body>) while the plugin reports success.
      const isVisibleEl = (el) => {
        if (el === null || el === undefined || !(el instanceof Element)) return false;
        try { return el.getClientRects().length > 0; } catch { return false; }
      };
      // Element scopes for keyed actions, most specific first: the focused
      // split-view pane, else the first VISIBLE pane, else the document. Hidden
      // panes are skipped so the standard chat UI is targeted when the workspace
      // is tucked away.
      const hotkeyScopes = () => {
        const scopes = [];
        try {
          const focusedPane = document.querySelector(".dsv-pane.focused");
          if (isVisibleEl(focusedPane)) {
            scopes.push(focusedPane);
          } else {
            const panes = document.querySelectorAll(".dsv-pane");
            for (const pane of panes) {
              if (isVisibleEl(pane)) { scopes.push(pane); break; }
            }
          }
        } catch {}
      ...
      // Draft of the composer input, on either core: textareas expose `.value`,
      // the 0.1.5 Lexical composer is a contenteditable div whose innerText is
      // "" while empty (its placeholder is an attribute, never content).
      const draftTextOf = (el) => {
      ...
      // Put the caret at the end of the draft. Textareas use setSelectionRange;
      // the contenteditable composer needs a Selection/Range collapse to end.
      const caretToEnd = (el) => {
        try {
          ...
          if (sel === null || typeof document.createRange !== "function") return;
      ...
      const focusComposerInput = () => {
        ...
        try {
          input.focus();
          if (!wasFocused) caretToEnd(input);
        } catch (err) {
      ...
      const chatScrollerEl = () => {
        for (const selector of CHAT_SCROLLER_SELECTORS) {
          try {
            ...
            for (const el of els) if (isVisibleEl(el)) return el;
          } catch {}
        }
        return null;
      };
      // True when the split-view workspace is actually on screen (Alt+W tucks
      // the whole overlay away while the panes stay mounted). Decides whether
      // transcript focus delegates to the split-view bridge or lands on the
      // standard chat scroller.
      const splitViewVisible = () => {
        try {
          const panes = document.querySelectorAll(".dsv-pane");
          for (const pane of panes) if (isVisibleEl(pane)) return true;
          return false;
        } catch { return false; }
      };
      // True when the element is a chat transcript scroller: the stock DSH
      // one (data-conversation-scroll), a chat view with data-chat-scroll, or
      // the split-view pane's focusable scroller (its ChatView scroller has
      // hashed CSS classes and no stable attribute, so the pane + tabindex
      // fingerprint is the only reliable marker).
      const isTranscriptScroller = (el) => {
        if (!(el instanceof Element)) return false;
        if (el.matches("[data-conversation-scroll],[data-chat-scroll]")) return true;
        return el.closest(".dsv-pane") !== null && el.getAttribute("tabindex") === "-1";
      };
      ...
      const focusChatTranscript = () => {
        ...
        try {
          const bridge = window.__dshSplitView;
          if (bridge !== undefined && bridge !== null && typeof bridge.focusChat === "function" && splitViewVisible() && bridge.focusChat() === true) {
            const focused = document.activeElement;
            if (focused instanceof Element) {
              focused.classList.add("shk-silent");
              try { focused.style.setProperty("outline", "none"); } catch {}
            }
        ...
        try {
          ...
          scroller.classList.add("shk-silent");
      ...
      const sendAlternate = () => {
        ...
        if (String(draftTextOf(input)).trim() === "") { showToast(t("sendAlternateEmpty"), "warn"); return; }
      ...
      const onKeydown = (event) => {
        ...
        try {
          ...
          for (const id of Object.keys(bindings)) {
            ...
            else if (id === "closeSplitView") closeSplitView();
      ...
      slots.inject("sidebar.footer.action", () => slots.register(
        { name: "sidebar.footer.action", id: "session-hotkeys-button", order: 40, label: t("appTitle") },
        FooterButton
      ));
    ...
    exports.apply = apply;
    // `slots` is needed at apply time (both slot registrations below). The
    // other services this plugin talks to are all provided ASYNCHRONOUSLY on
    // DSH 0.1.5 — `sessions` (dsh-api-session-controller, behind
    // connection/remote), `workspaces` (dsh-api-workspace-controller, behind
    // remote) and `locale` (behind settingsScope) all arrive later than slots.
    // Without declaring them here, the apply-time captures in `apply` land on
    // `undefined` and every session/workspace/locale feature silently disables
    // itself (the 0.1.5 regression: Alt+1 answers "Sessions service
    // unavailable"). cordis defers apply() until every injected service is
    // provided. `uiWorkspace` is deliberately NOT injected — it does not exist
    // on 0.1.1 — and is resolved lazily at keypress time instead.
